...
bba::Operations bba::parse_arguments(int argc, char *argv[])
{
    ...
    for (; i < arguments.size(); i++)
    {
        ...
        else if (action == "--interpolation")
        {
            ...
            else //VIDEOIN UND VIDEOOUT SIND OBEN BEI INPUT UND OUTPUT
            {
        ...
        {
            Operation op;
            op.action = bba::ACTION_TRACK;
            op.values = consumeNextNArguments(bba::NUM_PARAMS_TRACK);
            op.trackText = consumeArgument();
            ops.push_back(op);
        }
        else if (action == "--track-gui")
        {
            Operation op;
            op.action = bba::ACTION_TRACK_GUI;
            op.values = consumeNextNArguments(bba::NUM_PARAMS_TRACK_GUI);
            op.trackText = consumeArgument();
            ops.push_back(op);
        }
        else
