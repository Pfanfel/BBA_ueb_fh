...
void util::writeToCout(cv::Mat &img)
{
    ...
    if (img.channels() > 1)
    {
        cv::imencode(".ppm", img, buff, std::vector<int>{cv::IMWRITE_PXM_BINARY, 0}); //Encodiert das Bild als .ppm und schreibt es in den buffer mit dem Flag auf 0
    }
    else
    {
        cv::imencode(".pgm", img, buff, std::vector<int>{cv::IMWRITE_PXM_BINARY, 0}); //Encodiert das Bild als .pgm und schreibt es in den buffer mit dem Flag auf 0
    }
